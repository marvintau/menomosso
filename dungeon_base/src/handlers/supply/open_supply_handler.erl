...
handle_post(Req, State) ->
    ...
    {[{<<"loot_id">>, LootID}]} = jiffy:decode(ReqBody),

    Payload = case dungeon_base_sup:query({open_supply,{LootID}}) of
    {ok, QueryRes} -> [#{loot_id=>LootIDRes, item_id=>ItemID, item_qty=>ItemQty, item_name=>ItemName, description=>Description} || {LootIDRes, ItemID, ItemQty, ItemName, Description} <- QueryRes];
        Err -> #{error => Err}
    end,
