...
-module(send_mail_attachment_handler).
...
handle_post(Req, State) ->
    ...
    #{<<"sender_id">>:=SenderID, <<"receiver_id">>:=ReceiverID, <<"content">>:=Content, <<"attachment">>:=Attachment} = jiffy:decode(ReqBody, [return_maps]),
    
    Message = dungeon_base_sup:query({send_mail_attached, {SenderID, ReceiverID, Content, Attachment}}),
